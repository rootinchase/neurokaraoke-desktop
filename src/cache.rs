...
impl Cache {
    ...
    pub async fn cache_pass(self: &Arc<Self>, client: Client, config: &CacheConfig) {
        ...
        .unwrap_or(false)
        {
            ...
            self.entries.retain(|key, entry| {
                ...
                if entry.last_touched == 0 || entry.last_touched >= expires {
                    true
                } else {
            ...
            if total > max {
                ...
                while total > max {
                    let value = iter
                        .next()
                        .expect("how did we run out of items to grab if the total size > 0?");
                    if value.1.0 == 0 {
                        continue;
                    }
                    total -= value.1.1;

                    let id = self.entries.remove(&value.0).unwrap().1.id;
                    let path = cache_dir().join(format!("assets/{:016x}", id));
                    let _ = tokio::fs::remove_file(path).await;
                }
            }
    ...
    pub fn create_worker<F: Future + Send + Sync + 'static>(
        self: Arc<Self>,
        mut extra_pass: impl FnMut() -> F + Send + 'static,
    ...
    pub async fn get_or_download_audio(
        ...
        if let Some(file) = self.get(&key).await {
            crate::debug_log!(
                "⚡ [Cache API] CACHE HIT: Playing local copy for UUID: {}",
                song_uuid
            );
            return Ok(file);
        }
        ...
        crate::debug_log!(
            "🌍 [Cache API] CACHE MISS: Downloading from remote endpoint: {}",
            url
        );
    ...
    pub async fn get_or_download_image(
        ...
        if content_type
            .as_deref()
            .map_or(true, |ct| !ct.starts_with("image/"))
        {
            let body_text = String::from_utf8_lossy(&body_bytes);
            crate::debug_log!(
                "🔴 [Cache Core HTTP] UNEXPECTED CONTENT-TYPE. Status: {}, Content-Type: {:?}, Body: {}",
                status,
                content_type,
                body_text
            );
        ...
        crate::debug_log!(
            "💾 [Cache Core HTTP] FLUSHING ASSET DATA. Size: {} bytes, Extension: {:?}",
            size,
            extension
        );
